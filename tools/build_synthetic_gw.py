...
sportsbet/SYNTHETIC_NOTE.txt so `--gw` archiving withholds player history until real odds land.
...
def main():
    ...
    with open(os.path.join(sb, "SYNTHETIC_NOTE.txt"), "w", encoding="utf-8") as fh:
        fh.write(f"Generated by tools/build_synthetic_gw.py --gw {a.gw} --factor-gw {a.factor_gw}. "
                 f"Synthetic F1 (GW{a.gw}) markets ONLY, from GW{a.factor_gw} actual factors x the new "
                 f"fixtures' win probs; F2+ are derived off F1 (no synthetic F2). Rerun tools/betway.py "
                 f"when real markets open (it overwrites each priced market); do not record --gw archives "
                 f"while synthetic.\n")

    print(f"\nSynthetic GW{a.gw} F1 markets written to sportsbet/*.csv (F1 only — F2 derived off F1); "
          f"_f2 files cleared; SYNTHETIC_NOTE.txt dropped. Run tools/betway.py to override with real odds.")
