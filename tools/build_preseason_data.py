...
Synthetic odds are placeholders: rerun tools/betway.py when real markets open (it
...
def synth_odds(season, lineups, fallback):
    ...
    # Provenance manifest (fpl_pipeline/provenance.py) is the single source of truth - no note file.
    # All PLAYER markets are model-derived placeholders pre-season, so the archive guard withholds
    # player history until betway.py / ladbrokes_cards.py flip them real. tools/odds_status.py shows state.
    provenance.reset_synthetic(1, [
...
def main(rebuild_lineups=False, lineups_only=False):
    ...
    print("Synthetic odds written to sportsbet/*.csv (provenance reset to synthetic - see tools/odds_status.py)")
