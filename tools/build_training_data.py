...
# Only OBJECTIVE inputs are features — no outputs of our own models (win_hat/opp_hat from win_pred,
# and difficulty_shift built on them, are deliberately excluded: they would silently go stale if we
# refit win_pred). The raw outright odds + diffs carry the same info objectively.
...
def main():
    ...
    for stat in STATS:
        ...
        out = sub[KEYS + FEATURES + ["predicted", "actual"]]
